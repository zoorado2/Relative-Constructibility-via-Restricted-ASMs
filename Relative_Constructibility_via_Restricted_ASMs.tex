...
\begin{quote}
    \emph{if $X$ nicely generalises a model among \ref{ct1} to \ref{ct3} and the Church-Turing thesis says that the object of generalisation is a model of computation, then $X$ should also be a model of (generalised) computation.}
\end{quote}
...
Perhaps the most celebrated attempt at amalgamating and formalising useful mental models of computations and algorithms, comes in the form of a series of papers (starting with \cite{gurevich}) by Gurevich on abstract state machines. Abstract state machines far more closely resemble ``high-level'' meta-theoretic representations of Plotkin-style semantics (found e.g. in \cite{plotkin}), than they do anything among \ref{ct1} to \ref{ct3}. That these machines have been adapted for use in fields ranging from software development to systems engineering via (what is now known as) the \textit{ASM method} is testament to the intuitiveness of their design. Historical details regarding the ASM method can be found in \cite{borger} and Chapter 9 of \cite{borgerbook}. Having said all that, how well abstract state machines are equipped to handle the philosophical demands of computing with infinite sets is yet to be vigorously tested, for their applications have so far been finitary in nature.

In this paper, we investigate the promise of abstract state machines in infinitary computations. We make well-meaning restrictions to their definition along the way, so that they are better endowed with our intuition of computation born out of classical computing. 
...
Unlike the previous models of generalised computation, abstract state machines were not designed with any form of relative computability in mind. In fact, in the name of generality, the definition of an abstract state machine does not even specify the existence of canonical inputs or outputs, which are typically integral to associating a relative computability relation with a computational model. Further, preliminary investigations into the limits of their computational power quickly uncover something philosophically unsatisfying, if not unsettling. We seek to correct this with our restrictions to the definition of abstract state machines.

The way restrictions are added to the notion of abstract state machines results in a new model of generalised computation called \textit{restricted abstract state machines with parameters} (abbreviated as \textit{RASMPs}). The main draw of RASMPs is that they possess more concrete guardrails than abstract state machines without compromising too much of the latter's intuitiveness. Coupled with an input/output paradigm, RASMPs naturally induce a relative computability relation, $\leq^P$, on all sets of ordinals. It turns out that $\leq^P$ coincides with both $\preceq_A$ as well as the \textit{relative constructibility} relation. Unlike $\preceq_A$ though, $\leq^P$ tends very much to remain transitive under computational constraints. This makes for interesting comparisons between $\leq^P$ and say, the relations in Table \ref{table1}, when size limitations are imposed.
...
\begin{table}[!htp]
    \caption{Comparing two of our relative computability relations with more standard ones in the of study generalised computability, at arbitrary admissible ordinals $\alpha$.}
    \label{table3}
    \centering
